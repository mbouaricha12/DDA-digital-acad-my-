...
const lessonM13Id = 'M1.3';
const lessonM13Meta = DDALearning.findLesson(DDA.curriculum, lessonM13Id);
const lessonM13Def = lessonM13Meta?.lesson || null;
if (lessonM13Def) {
  document.getElementById('lesson-m13-main').insertAdjacentHTML('beforeend', DDALessonRenderer.renderLessonMain(lessonM13Meta.module, lessonM13Def, 'm13'));
  document.getElementById('lesson-m13-outline').innerHTML = DDALessonRenderer.renderLessonOutline(lessonM13Def, 'm13');
}
...
const viewPermissions = { dashboard: 'dashboard', path: 'path', lesson: 'lesson_m01', 'lesson-m02': 'lesson_m01', 'lesson-m03': 'lesson_m01', 'lesson-m11': 'lesson_m01', 'lesson-m12': 'lesson_m01', 'lesson-m13': 'lesson_m01', progress: 'progress', journal: 'journal', resources: 'resources_free', markets: 'market_room', brokers: 'broker_hub', membership: 'membership', support: 'support', profile: 'profile' };
...
const LESSON_VIEW_ID = { 'M0.1': 'lesson', 'M0.2': 'lesson-m02', 'M0.3': 'lesson-m03', 'M1.1': 'lesson-m11', 'M1.2': 'lesson-m12', 'M1.3': 'lesson-m13' };
...
function showView(id, recordEvent = true) {
  ...
  document.body.classList.toggle('lesson-focus', id === 'lesson' || id === 'lesson-m02' || id === 'lesson-m03' || id === 'lesson-m11' || id === 'lesson-m12' || id === 'lesson-m13');
...
if (lessonM13Def) bindMarkUnderstood(lessonM13Id, { buttonId: 'mark-understood-m13', savedStateId: 'saved-state-m13', scrollToId: 'm13-exercise-case-block' });
...
if (lessonM13Def) {
  const m13Hedger = findLessonBlock(lessonM13Def, 'm13-hedger');
  const m13Investor = findLessonBlock(lessonM13Def, 'm13-investor');
  const m13Liquidity = findLessonBlock(lessonM13Def, 'm13-liquidity');
  const m13Exercise = findLessonBlock(lessonM13Def, 'm13-exercise');
  const m13Quiz = findLessonBlock(lessonM13Def, 'm13-quiz');
  bindQuestion(lessonM13Id, { role: 'practice', name: m13Hedger.id, successText: 'Bonne lecture du rôle de couverture.' });
  bindQuestion(lessonM13Id, { role: 'practice', name: m13Investor.id, successText: 'Bonne lecture de l’horizon investisseur.' });
  bindQuestion(lessonM13Id, { role: 'practice', name: m13Liquidity.id, successText: 'Bonne lecture du rôle de liquidité.' });
  bindQuestion(lessonM13Id, { role: 'exercise', name: m13Exercise.id, successText: lessonM13Def.practice.successText }, { gateId: 'm13-quiz-block' });
  bindQuestion(lessonM13Id, { role: 'quiz', name: m13Quiz.data.id, successText: m13Quiz.data.successText }, { resultId: 'result-card-m13', savedStateId: 'saved-state-m13' });
}
...
function resetPilot() {
  ...
  const m13Result = document.getElementById('result-card-m13');
  if (m13Result) m13Result.hidden = true;
  renderState();
