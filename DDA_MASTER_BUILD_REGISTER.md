...
| Apprenant | Résumé / Exercice / Quiz | CONSTRUIT | M0.1 avec déverrouillage progressif ; rythme éditorial ajouté (directive §8-9) : stepper d'étapes visible mobile+desktop reflétant l'état réel (Comprendre→Exercice→Quiz→Résultat), bloc de comparaison renforçant le principe déjà énoncé (aucune notion nouvelle inventée), feedback d'erreur propre à chaque mauvaise réponse (exercice et quiz) au lieu d'un message générique unique |
